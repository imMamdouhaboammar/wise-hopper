...
export default async function AccountPage({
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 xl:px-12 py-10 sm:py-16 space-y-10" dir="rtl">
      {/* Page Title & Breadcrumb */}
      <div className="space-y-3">
        <div className="inline-flex items-center gap-2 px-3 py-1 bg-lavender/70 border border-lavender-border/60 rounded-full text-xs font-semibold text-primary">
          <User className="w-3.5 h-3.5" />
          <span>مركز تحكم القارئ</span>
        </div>
        <h1 className="text-2xl sm:text-3xl lg:text-4xl font-bold text-ink-primary tracking-tight">
      ...
      {logged_out && (
        <div className="p-4 bg-emerald-50 border border-emerald-200/80 text-emerald-800 rounded-2xl text-xs sm:text-sm font-medium flex items-center gap-3 animate-in fade-in duration-200">
      ...
      {canceled && (
        <div className="p-4 bg-amber-50 border border-amber-200/80 text-amber-800 rounded-2xl text-xs sm:text-sm font-medium flex items-center gap-3 animate-in fade-in duration-200">
      ...
      {session.isOwner ? (
        <div className="p-6 sm:p-8 bg-white rounded-3xl border border-primary/20 shadow-soft-sm relative overflow-hidden">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6 relative z-10">
            <div className="space-y-2">
              <div className="flex items-center gap-2">
                <span className="inline-flex items-center gap-1.5 px-2.5 py-0.5 bg-primary text-white text-xs font-bold rounded-full">
              ...
              <p className="text-xs sm:text-sm text-ink-secondary leading-relaxed max-w-lg">
            ...
            <div className="flex items-center gap-3 shrink-0">
              <Link
                href="/studio"
                className="px-5 py-2.5 bg-primary hover:bg-primary-hover text-white text-xs sm:text-sm font-bold rounded-xl shadow-soft-xs transition-all flex items-center gap-2"
              >
              ...
              <form action="/api/studio/auth/logout" method="POST">
                <button
                  type="submit"
                  className="px-4 py-2.5 bg-white hover:bg-rose-50 text-rose-600 border border-rose-200 text-xs sm:text-sm font-semibold rounded-xl transition-colors cursor-pointer flex items-center gap-1.5"
                >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-10">
        <div className="lg:col-span-8 space-y-8">
          ...
          <div className="bg-white rounded-3xl p-6 sm:p-8 border border-lavender-border/70 shadow-soft-xs space-y-6">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-2xl bg-lavender/70 text-primary flex items-center justify-center font-bold">
            ...
            {/* Meta Table */}
            <div className="space-y-3 text-xs sm:text-sm text-ink-secondary border-b border-lavender-border/60 pb-6">
            ...
            <div className="flex flex-wrap items-center gap-3">
              <Link
                href="/membership"
                className="px-5 py-2.5 bg-lavender text-primary hover:bg-lavender-dark text-xs sm:text-sm font-bold rounded-xl transition-colors"
              >
              ...
              <form action="/api/billing/cancel" method="POST">
                <button
                  type="submit"
                  className="px-5 py-2.5 bg-white text-rose-600 hover:bg-rose-50 border border-rose-200 text-xs sm:text-sm font-semibold rounded-xl transition-colors cursor-pointer"
                >
            ...
            <form action="/api/newsletter/update-topics" method="POST" className="space-y-5">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {topics.map((t) => (
                  <label
                    key={t.id}
                    className="flex items-start gap-3 p-3.5 rounded-2xl border border-lavender-border/70 hover:bg-lavender-light/30 transition-colors cursor-pointer"
                  >
                    <input
                      type="checkbox"
                      name="topics"
                    ...
                    <div className="space-y-0.5">
                      <span className="text-xs sm:text-sm font-bold text-ink-primary block">{t.name}</span>
                      <span className="text-xs text-ink-secondary block line-clamp-1">
              ...
              <div className="pt-4 border-t border-lavender-border/60 flex items-center justify-between">
                <button
                  type="submit"
                  className="px-6 py-2.5 bg-primary hover:bg-primary-hover text-white text-xs sm:text-sm font-bold rounded-xl transition-colors cursor-pointer shadow-soft-xs"
                >
                  حفظ التفضيلات
                </button>
        ...
        <div className="lg:col-span-4 space-y-6">
          ...
          <div className="bg-white rounded-3xl p-6 border border-lavender-border/70 shadow-soft-xs space-y-4">
            <div className="flex items-center gap-2.5 pb-3 border-b border-lavender-border/60">
              <User className="w-4 h-4 text-primary" />
              <h3 className="font-bold text-sm text-ink-primary">هوية القارئ</h3>
            </div>

            <div className="space-y-3 text-xs sm:text-sm">
              <div>
                <span className="text-xs text-ink-secondary block mb-1">البريد الإلكتروني المعتمد:</span>
                <span className="font-mono text-xs sm:text-sm text-ink-primary font-semibold block truncate">
                  reader@wise-hopper.io
                </span>
              </div>
              <div>
                <span className="text-xs text-ink-secondary block mb-1">نوع المصادقة:</span>
                <span className="text-xs sm:text-sm font-semibold text-ink-primary flex items-center gap-1.5">
          ...
          {/* Reader Support Card */}
          <div className="bg-lavender-light/60 rounded-3xl p-6 border border-lavender-border/70 shadow-soft-xs space-y-3">
            ...
            <a
              ...
              className="inline-flex items-center gap-1 text-xs sm:text-sm font-bold text-primary hover:text-primary-hover transition-colors"
            >
